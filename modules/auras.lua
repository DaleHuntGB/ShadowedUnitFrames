...
local function updateButton(id, group, config)
	...
	if( not button ) then
		...
		button.cooldown:SetFrameLevel(9)
		button.cooldown:SetSwipeColor(0, 0, 0, 0.75)
		-- Make sure that cooldown frame is lower than button stacks.
		button.cooldown:Hide()
		button.stack = button:CreateFontString(nil, "OVERLAY")
		button.stack:SetFont("Fonts\\FRIZQT__.ttf", 12, "OUTLINE") -- Blizzard Default Font.
		button.stack:SetShadowColor(0, 0, 0, 0) -- Remove Shadow.
		button.stack:SetShadowOffset(0.00, 0.00) -- Remove Shadow.
		button.stack:SetHeight(1)
		button.stack:SetWidth(1)
		button.stack:SetAllPoints(button)
		button.stack:SetJustifyV("BOTTOM")
		button.stack:SetJustifyH("RIGHT")
		if config.size <= 38 then 
			button.stack:SetPoint("BOTTOMRIGHT", button, "BOTTOMRIGHT", 0, 2)
		else
			button.stack:SetPoint("BOTTOMRIGHT", button, "BOTTOMRIGHT", 0, 2)
		end
		-- Position the icon text correctly when the button is smaller.
	...
	elseif( ShadowUF.db.profile.auras.borderType == "blizzard" ) then
		...
		button.border:Show()
	elseif (ShadowUF.db.profile.auras.borderType == "1px") then
		if config.size <= 38 then
			button.border:SetTexture("Interface\\AddOns\\ShadowedUnitFrames\\media\\textures\\Border64.tga")
		else
			button.border:SetTexture("Interface\\AddOns\\ShadowedUnitFrames\\media\\textures\\Border128.tga")
		end
		-- This will maintain a 1px Border for all icon sizes.
...
local function renderAura(parent, frame, type, config, displayConfig, index, filter, isFriendly, curable, name, texture, count, auraType, duration, endTime, caster, isRemovable, nameplateShowPersonal, spellID, canApplyAura, isBossDebuff)
	...
	elseif( ( type == "debuffs" ) and not ShadowUF.db.profile.auras.disableColor ) then
