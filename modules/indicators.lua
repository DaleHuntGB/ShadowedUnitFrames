...
function Indicators:UpdateLFDRole(frame, event)
	...
	-- Replaced the above with the below to use the new LFG role icons. Will eventually add an option here :)
	if( role == "TANK" ) then
		frame.indicators.lfdRole:SetTexture("Interface\\AddOns\\ShadowedUnitFrames\\media\\textures\\Tank.tga")
		frame.indicators.lfdRole:Show()
	elseif( role == "HEALER" ) then
		frame.indicators.lfdRole:SetTexture("Interface\\AddOns\\ShadowedUnitFrames\\media\\textures\\Healer.tga")
		frame.indicators.lfdRole:Show()
	elseif( role == "DAMAGER" ) then
		frame.indicators.lfdRole:SetTexture("Interface\\AddOns\\ShadowedUnitFrames\\media\\textures\\DPS.tga")
		frame.indicators.lfdRole:Show()
	else
		frame.indicators.lfdRole:Hide()
	end
