...
local function getMaxRange(unit)
	local minRange, maxRange = RC:GetRange(unit, true, true)
...
local function checkRange(self)
	...
	-- Nope, fall back to interaction :(
		...
		if UnitCanAttack('player', frame.unit) or UnitIsUnit(frame.unit, 'pet') then
			...
			frame:SetRangeAlpha((UnitIsConnected(frame.unit) and friendlyIsInRange(frame.unit)) and ShadowUF.db.profile.units[frame.unitType].range.inAlpha or ShadowUF.db.profile.units[frame.unitType].range.oorAlpha)
		end
	end
end
